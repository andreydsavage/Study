...
def train_epoch(model, optimizer, dataset_train, batch_size=64,
                print_loss_every_n_batches=500, device='cpu'):
    """
    ...
    :return: None
    """
    train_generator = batches_generator(dataset_train, batch_size=batch_size, 
...
def eval_model(model, dataset_val, batch_size=32, device=None) -> float:
    ...
    preds = []
    targets = []
    val_generator = batches_generator(dataset_val, batch_size=batch_size,
                                      device=device, is_train=True, output_format='torch')
    model.eval()
...
def inference(model, dataset_test, batch_size=32, device=None) -> pd.DataFrame:
    ...
    preds = []
    user_ids = []
    test_generator = batches_generator(dataset_test, batch_size=batch_size,
                                       verbose=False, device=device, is_train=False,
                                       output_format='torch')

    for batch in tqdm(test_generator, desc='Test time predictions'):
        user_ids.extend(batch['uesr_id'])
        output = model(batch['transactions_features']).sigmoid()
        preds.extend(output.detach().cpu().numpy().flatten())
